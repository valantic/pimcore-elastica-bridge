...
- If you extend or instantiate `\Valantic\ElasticaBridgeBundle\Command\Cleanup`, its constructor now takes a `\Valantic\ElasticaBridgeBundle\Service\CleanupService` instead of the Elasticsearch client and index repository
- Added `\Valantic\ElasticaBridgeBundle\Service\CleanupService::cleanUp` to programmatically delete indices and aliases; it returns a `\Valantic\ElasticaBridgeBundle\Model\CleanupResult` [#92](https://github.com/valantic/pimcore-elastica-bridge/pull/92)
